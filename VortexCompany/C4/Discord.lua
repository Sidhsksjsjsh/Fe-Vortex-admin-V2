...
--[[
-- local WarningGenerator = game:GetService("CoreGui")
...
function BP_Prompt(title, Message, buttonname)
    if getrenv then
        local ErrorPrompt = getrenv().require(game:GetService("CoreGui"):WaitForChild("RobloxGui"):WaitForChild("Modules"):WaitForChild("ErrorPrompt")) -- File can be located in your roblox folder (C:\Users\%Username%\AppData\Local\Roblox\Versions\whateverversionitis\ExtraContent\scripts\CoreScripts\Modules)
        local prompt = ErrorPrompt.new("Default",{HideErrorCode = true})
        local ErrorStoarge = Create("ScreenGui",{Parent = game:GetService("CoreGui"),ResetOnSpawn = false})
...
function GetPlayerInfoByScript(dataType)
    if dataType == "getBio" then
       _G.DataInfoType.user = game:HttpGet("https://users.roblox.com/v1/users/"..speaker.UserId)
      _G.DataInfoType.json = HttpService:JSONDecode(_G.DataInfoType.user)
      _G.DataInfoType.bio = _G.DataInfoType.json["bio"]
      table.insert(_G.API_EXPLOIT.UserBio, _G.DataInfoType.bio)
       return table.concat(_G.API_EXPLOIT.UserBio, ', ')
	elseif dataType == "getCreated" then
   	    _G.DataInfoType.user = game:HttpGet("https://users.roblox.com/v1/users/"..speaker.UserId)
	    _G.DataInfoType.json = HttpService:JSONDecode(_G.DataInfoType.user)
	    _G.DataInfoType.date = _G.DataInfoType.json["created"]:sub(1,10)
	    _G.DataInfoType.splitDates = string.split(_G.API_EXPLOIT.UserDate,"-")
	        table.insert(_G.API_EXPLOIT.UserDate,tostring(_G.DataInfoType.splitDates[2]) .. "/" .. tostring(_G.DataInfoType.splitDates[3]) .. "/" .. tostring(_G.DataInfoType.splitDates[1]))
	return table.concat(_G.API_EXPLOIT.UserDate, ', ')
	else
        ErrorPrompt("Error or invalid JSON type","sorry.")
    end
end
