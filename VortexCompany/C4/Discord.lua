...
-- (62x) - final: 100 JSKV5
local version = "-MAX | VortexOS V6.0.3" -- reverted version
...
local GuiService = Vortex:Service("GuiService")
local HttpService = Vortex:Service("HttpService")
local RunService = Vortex:Service("RunService")
local hb = RunService.Heartbeat
local VoiceChatService = Vortex:Service("VoiceChatService")
local vim = Vortex:Service("VirtualInputManager")
...
local workspace = Vortex:Service("Workspace")
local oofing = false
local TeleportService = Vortex:Service("TeleportService")
...
local walkto = false
local NetworkClient = Vortex:Service("NetworkClient")
local currentToolSize = ""
local PathService = Vortex:Service("PathfindingService")
...
local ProtocolCD = 1
local UserInputService = Vortex:Service("UserInputService")
local ContextActionService = Vortex:Service("ContextActionService")
local gotopartDelay = 0.1
local TweenService = Vortex:Service("TweenService")
local countb = 0
local HumanModCons = {}
local frozenParts = {}
...
local UrlScript = {
	...
	"https://raw.githubusercontent.com/Turtle-Secure/Anime-Infinity/main/.lua"
}
...
if AutoSendScript == false then
	if BypassWord then
	      ...
	      --Vortex:TranslatorAPI(TextBar.Text:gsub("ass",Bypasserword["ass"]):gsub("dick",Bypasserword["dick"]):gsub("pussy",Bypasserword["pussy"]):gsub("shit",Bypasserword["shit"]):gsub("fuck",Bypasserword["fuck"]):gsub("asshole",Bypasserword["asshole"]):gsub("nigga",Bypasserword["nigga"]):gsub("suck",Bypasserword["suck"]):gsub("discord",Bypasserword["discord"]):gsub("tiktok",Bypasserword["tiktok"]):gsub("cum",Bypasserword["cum"]):gsub("condom",Bypasserword["condom"]):gsub("sex",Bypasserword["sex"]):gsub("sexy",Bypasserword["sexy"]):gsub("sperm",Bypasserword["sperm"]):gsub("tits",Bypasserword["tits"]):gsub("boobs",Bypasserword["boobs"]):gsub("penis",Bypasserword["penis"]):gsub("nigger",Bypasserword["nigger"]):gsub("i dont give a",Bypasserword["idga"]))
	else
	   Vortex:SendMessage(TextBar.Text:gsub("ass",Bypasserword["ass"]):gsub("dick",Bypasserword["dick"]):gsub("pussy",Bypasserword["pussy"]):gsub("shit",Bypasserword["shit"]):gsub("fuck",Bypasserword["fuck"]):gsub("asshole",Bypasserword["asshole"]):gsub("nigga",Bypasserword["nigga"]):gsub("suck",Bypasserword["suck"]):gsub("discord",Bypasserword["discord"]):gsub("tiktok",Bypasserword["tiktok"]):gsub("cum",Bypasserword["cum"]):gsub("condom",Bypasserword["condom"]):gsub("sex",Bypasserword["sex"]):gsub("sexy",Bypasserword["sexy"]):gsub("sperm",Bypasserword["sperm"]):gsub("tits",Bypasserword["tits"]):gsub("boobs",Bypasserword["boobs"]):gsub("penis",Bypasserword["penis"]):gsub("nigger",Bypasserword["nigger"]):gsub("i dont give a",Bypasserword["idga"]))
	   --Vortex:TranslatorAPI(TextBar.Text:gsub("ass",Bypasserword["ass"]):gsub("dick",Bypasserword["dick"]):gsub("pussy",Bypasserword["pussy"]):gsub("shit",Bypasserword["shit"]):gsub("fuck",Bypasserword["fuck"]):gsub("asshole",Bypasserword["asshole"]):gsub("nigga",Bypasserword["nigga"]):gsub("suck",Bypasserword["suck"]):gsub("discord",Bypasserword["discord"]):gsub("tiktok",Bypasserword["tiktok"]):gsub("cum",Bypasserword["cum"]):gsub("condom",Bypasserword["condom"]):gsub("sex",Bypasserword["sex"]):gsub("sexy",Bypasserword["sexy"]):gsub("sperm",Bypasserword["sperm"]):gsub("tits",Bypasserword["tits"]):gsub("boobs",Bypasserword["boobs"]):gsub("penis",Bypasserword["penis"]):gsub("nigger",Bypasserword["nigger"]):gsub("i dont give a",Bypasserword["idga"]))
    end
...
function DisplayUI(UIname)
      ...
      dragify(TextBar)
      Vortex:PromptUI(":29889: Fatal Error Detected")
   end
      if UIname == "exc" then
      TextBarExecutor.Visible = true
      BarButtonExecute.Visible = true
...
local var = string.sub(msg,space+1)
	if var then
		if var:lower() == "reset" then
			...
			local coreGuiType = coreGuiTypeNames[var:gsub(" ","")]
			if coreGuiType then
...
if (not _G.UrlList[tonumber(var)]) then
    ...
    copy(string.format('-- Generated by AI and API \n-- AI by : Tora4172#0 \n\n{\n    "apiMetadata": {\n        "websiteName": "%s",\n        "baseUrl": "%s",\n        "documentation": "%s"\n    },\n    "requestDetails": {\n        "endpoint": "/users/info",\n        "method": "GET",\n        "host": "%s",\n        "cookie": {\n            "SESSIONID": "%s"\n        },\n        "fingerprint": "%s",\n        "userAgent": "%s"\n    },\n    "response": {\n        "statusCode": 200,\n        "statusMessage": "OK",\n        "data": {\n            "userId": "%s",\n            "username": "%s",\n            "displayName": "%s",\n            "email": "%s",\n            "dateJoined": "%s",\n            "bio": "%s"\n        }\n    },\n    "meta": {\n        "timestamp": "%s",\n        "version": "5.0.0",\n        "serverId": "MS-API-01"\n    }\n}',"nil",_G.UrlList[tonumber(var)].urlHook,"nil",_G.UrlList[tonumber(var)].urlHook:gsub("githubusercontent","github"):gsub("pastebin","pastebin"):gsub("luarmor","luarmor"),HttpService:GenerateGUID(false),HttpService:GenerateGUID(false),Executor(),speaker.UserId,speaker.Name,speaker.DisplayName,"nil","{}","{}",os.date()))
end
end
if cmd == "cframe" then
local var = string.sub(msg,space+1)
...
if cmd == "dance" then -- shitty command
...
if cmd == "megahideandseek" or cmd == "mhas" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
...
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
        executeHTTPS(UrlScript[7])
end)
end
if cmd == "pubgsim" or cmd == "ps" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
...
if cmd == "shootafriends" or cmd == "saf" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
        executeHTTPS(UrlScript[15])
end)
end
if cmd == "possessor" or cmd == "pos" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
        executeHTTPS(UrlScript[16])
end)
end
if cmd == "shootwall" or cmd == "sws" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
...
if cmd == "sbs" or cmd == "shootbeam" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
        executeHTTPS(UrlScript[29])
end)
end
if cmd == "adf" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
        executeHTTPS(UrlScript[30])
end)
end
if cmd == "gb" or cmd == "gunball" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
...
if cmd == "fai" or cmd == "animeinf" then
Vortex:CheckError(function()
	notify("Injecting...","pls wait..")
...
cmds[#cmds + 1] = {Text = "[343] " .. tostring(prefix) .. "fai / animeinf",Title = "Run Free Anime Infinity Script"}
